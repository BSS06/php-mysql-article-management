...
class Angajat {
    ...
    private function checkAge($virsta) {
        return ($virsta >= 1 && $virsta <= 100);
    }
    public function setVirsta($virsta) {
        if ($this->checkAge($virsta)) {
            $this->virsta = $virsta;
        } else {
            echo "Varsta {$virsta} nu este corecta. Varsta nu a fost modificata.\n";
        }
    }
}
$ion = new Angajat();
