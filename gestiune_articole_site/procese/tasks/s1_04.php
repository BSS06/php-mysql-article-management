<?php
class Trapez {
    private $x1, $y1, $x2, $y2, $x3, $y3, $x4, $y4;
    ...
    public function getLaturaCD() { return $this->distanta($this->x3, $this->y3, $this->x4, $this->y4); }
    public function getLaturaDA() { return $this->distanta($this->x4, $this->y4, $this->x1, $this->y1); }
    public function getPerimetru() { return $this->getLaturaAB() + $this->getLaturaBC() + $this->getLaturaCD() + $this->getLaturaDA(); }
    public function getSuprafata() {
        $b1 = $this->getLaturaAB(); $b2 = $this->getLaturaCD();
        $A = $this->y2 - $this->y1; $B = $this->x1 - $this->x2; $C = $this->x2 * $this->y1 - $this->x1 * $this->y2;
        $h = abs($A * $this->x3 + $B * $this->y3 + $C) / sqrt($A * $A + $B * $B);
        return ($b1 + $b2) * $h / 2;
    }
    public function esteIsoscel() { return round($this->getLaturaBC(), 6) === round($this->getLaturaDA(), 6); }
...
$published = max(4, (int)($ctx['scope']['counts']['published_count'] ?? 4));
$draft = max(2, (int)($ctx['scope']['counts']['draft_count'] ?? 2));
$t = new Trapez(0, 0, $published, 0, $published - 1, $draft + 2, 1, $draft + 2);
$t->afiseaza();
