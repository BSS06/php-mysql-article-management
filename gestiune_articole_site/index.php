...
.sw-trigger:hover .u-avatar{opacity:.8}
...
<aside class="sb">
  ...
  <div class="user-card" style="padding:0;flex-direction:column;align-items:stretch;gap:0">
    <div class="sw-wrap" id="swWrap">
      <!-- Trigger: click pe user → deschide popup -->
      ...
      <div class="sw-popup">
        ...
        <a href="logout.php" class="sw-out">
          <span style="font-size:13px">⏻</span> Deconectare
        </a>
      </div>
    </div>
    <?php if(in_array($rol, ['administrator','redactor'], true)): ?>
    <div style="display:flex;gap:0;border-top:1px solid #1f2235">
      <a href="crud.php" class="logout-btn" title="Modul CRUD" style="color:#7c6fcd;flex:1;text-align:center;padding:7px">⚙ CRUD</a>
      <a href="obiective_editoriale.php" class="logout-btn" title="POO editorial" style="color:#a99ee8;flex:1;text-align:center;padding:7px;border-left:1px solid #1f2235">POO</a>
    </div>
    <?php endif; ?>
  </div>
<script>
...
function buildNav(f) {
  ...
  const flush = () => {
    ...
    if (group.items.length) {           
      chunks.push(`<div class="ng">${group.title}</div>`);
  ...
  MENU.forEach(m => {
    if (m.g) {
      flush();
      group = {title:m.g,items:[]};
      return;
    }
    if (f && !m.lbl.toLowerCase().includes(f)) return;
    const on = m.id === cur ? 'on' : '';
    const item = `<div class="ni ${on}" data-id="${m.id}"><div class="ni-dot"></div><span class="ni-lbl">${m.lbl}</span><span class="ni-bg ${m.bg}">${m.bg==='b1'?'DB1':m.bg==='b2'?'DB2':m.bg==='b3'?'DB3':m.bg==='bm'?'META':'×'}</span></div>`;
    if (group) group.items.push(item); else chunks.push(item);
  });
  flush();
  document.getElementById('nav').innerHTML = chunks.join('');
...
function tbl(cols, rows) {
  ...
  const HIDDEN_FOR_READER = ['id_autor','id_articol','id_utilizator','id_categorie',
                              'id_rol','id_comentariu','id_evaluare','id_vizualizare'];
  if (ROL === 'cititor') cols = cols.filter(c => !HIDDEN_FOR_READER.includes(c));
...
function kpiCards(v) {
  ...
  const it = ROL === 'cititor' ? [
    {v:gv('publicate','articole_publicate'),  l:'Articole publicate', s:'disponibile public',  ac:true},
    {v:gv('comentarii','total_comentarii'),   l:'Comentarii',         s:'aprobate',            ac:false},
    {v:gv('medie_nota','medie_evaluari'),     l:'Medie evaluări',     s:'din 5 stele',         ac:true},
    {v:gv('vizualizari','total_vizualizari'), l:'Vizualizări totale', s:'înregistrate',        ac:false},
  ] : [
...
async function loadPage(id, extra) {
  ...
  try {
      ...
      if (ROL === 'cititor') {
        ...
        setTimeout(() => {
          ...
          if(d.chart_stat){
            ...
            if(cv5){ const ch5 = new Chart(cv5,{type:'line',data:{labels:d.chart_stat.etichete,datasets:[{data:d.chart_stat.valori,borderColor:d.chart_stat.culoare,backgroundColor:d.chart_stat.culoare+'22',fill:true,tension:0.3,pointRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true},x:{ticks:{font:{size:11}}}}}}); charts.push(ch5); }
          }
        }, 60);
        return;
      }
    ...
    c.innerHTML = `
      ...
      ${ROL !== 'cititor' ? `<div class="r1 sql-wrap">
      ...
      ${(d.kpi && ROL !== 'cititor')?`<div class="kbox"><div class="klbl">Impact KPI</div><p><strong>→</strong> ${d.kpi}</p></div>`:''}`;
