...
if ($ACT === 'citeste') {
    ...
    if ($TBL === 'roluri') {
        if (!$IS_ADMIN) jserr('Acces interzis');
        jsok('ok', q(DB1, "SELECT r.id_rol,r.denumire_rol,r.descriere,COUNT(u.id_utilizator) AS nr_utilizatori, JSON_OBJECT('id',r.id_rol,'rol',r.denumire_rol,'nr',COUNT(u.id_utilizator)) AS metadata_json FROM roluri r LEFT JOIN utilizatori u USING(id_rol) GROUP BY r.id_rol"));
    }
    if ($TBL === 'articole') {
    ...
    if ($TBL === 'comentarii') {
        $w = $IS_ADMIN ? '' : "WHERE cm.id_utilizator = $UID";
...
if ($ACT === 'test_integritate') {
    ...
    // Dezactiveaza exceptiile mysqli - testele verifica return values, nu exceptii
    mysqli_report(MYSQLI_REPORT_OFF);

    try {
        // Sterge inregistrarile ramase din rulari anterioare
    ...
    ob_end_clean(); // sterge orice output accidental
